...
Core wrappers such as `Signer{:rs}`, `Program<T>{:rs}`, `SystemAccount{:rs}`, `UncheckedAccount{:rs}`, and `Sysvar<T>{:rs}` surface account metadata through trait impls. SPL wrappers in `anchor-spl` do the same from the downstream crate.
