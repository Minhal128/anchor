...
`ctx.remaining_accounts(){:rs}` returns `Result<Vec<AccountView>>{:rs}`, so unwrap it with `?{:rs}`. It walks the cursor only on first access and caches the result for later calls in the same instruction.
