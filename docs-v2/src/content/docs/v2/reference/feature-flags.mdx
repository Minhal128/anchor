---
title: Feature flags
description: 'Reference for the `alloc{:toml}`, `guardrails{:toml}`, `const-rent{:toml}`, `compat{:toml}`, and `testing{:toml}` feature flags on `anchor-lang`.'
---

`anchor-lang` ships five feature flags, two of which are on by default and the other three opt-in. Most programs never need to touch them, but tuning the set is useful for trimming binary size and CU cost in production builds.
...
| `testing{:toml}` | off | Enables `anchor_lang::testing{:rs}` with mock account scaffolding. |

## `alloc{:toml}` (default)

The `alloc{:toml}` feature enables `extern crate alloc{:rs}` and re-exports the `alloc{:rs}` crate under `anchor_lang::__alloc{:rs}`. Generated macro code reaches `Vec{:rs}`, `String{:rs}`, and other heap-allocated types through that re-export, so user crates do not have to import `alloc{:rs}` themselves.
...
[dependencies]
anchor-lang = { git = "...", branch = "anchor-next", default-features = false, features = ["alloc"] }
```
...
The `testing{:toml}` feature exposes the `anchor_lang::testing{:rs}` module, which provides stack-backed mock pinocchio types (such as `AccountView{:rs}` and the SBF input buffer) that the `lang-v2/tests/{:dir}` integration tests and Miri witnesses depend on. Enabling `testing{:toml}` also turns on `const-rent{:toml}` (so host-side tests can exercise rent-dependent helpers without needing SVM sysvar support) and `compat{:toml}` (so tests can cover the v1-style raw remaining-account shims):

```console showLineNumbers=false
$ <blue>cargo</blue> test <dim>-p</dim> anchor-lang <dim>--features</dim> testing
```
...
```toml title="Cargo.toml" showLineNumbers=false
[dependencies]
anchor-lang = { git = "...", branch = "anchor-next", default-features = false, features = ["alloc", "const-rent"] }
```
