...
`anchor codama convert` converts an Anchor IDL into a Codama IDL. `anchor
codama generate` converts the IDL and invokes the Codama renderer packages for
...
local network and runs the test. By default, [Surfpool](https://github.com/solana-foundation/surfpool)
